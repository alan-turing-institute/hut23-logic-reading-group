...
$\vee$, $\to$, $\leftrightarrow$, and $\neg$)]{connectives}.  A \defn{sentence} (of
...
this is not a tradition followed by the book) to omit many parentheses
when the result can be disambiguated by the following rules:
\begin{enumerate}
...
\subsubsection*{JG is confused about\dots}
\begin{enumerate}
\item
  ...
  \begin{enumerate}
  \item\label{item:shorthand} it is a mere shorthand for a specific,
...
\item What in fact is a sentence?  There seem to me to be two
  possibilities:
  \begin{enumerate}
