...
\newcommand{\fa}{\textcolor{gray}{\ensuremath{\mathbb{f}}}}
%% 
...
A \defn{truth value} is one of `$\tr$' (true) or `$\fa$' (false). (The
lighter shade for $\fa$ is just to make it clearer which is which.)
Sentences are the kind of thing that may have truth values.  A
\defn{valuation} is an assignment of a truth value to each of a set of
sentence letters.  A \defn{truth table} for a sentence is a set of rows,
each giving a possible valuation of the sentence letters in the sentence
together with the value of the whole sentence.  (The truth value of a
compound sentence is determined entirely by the truth values of its
constituent parts and the particular connectives.)  See
figure~\ref{fig:truthtables} for complete truth tables of the standard
connectives.
\begin{figure}[ht]
...
We now recapitulate the ideas of section~\ref{sec:english}.
...
Sentences $\wff{A}_1, \wff{A}_2, \dotsc, \wff{A}_n$ \defn{entail} sentence $\wff{C}$ if there is no valuation that makes all the $\wff{A}_i$ true and $\wff{C}$ false.  If indeed the $\wff{A}_i$ entail $\wff{C}$ we write
\begin{equation*}
